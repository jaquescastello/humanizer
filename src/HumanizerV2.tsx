...
export default function HumanizerV2() {
  ...
  async function generate() {
    ...
    try {
      ...
      const models3 = [...selectedModels];
      const lastHotSeatRound: Record<number, number> = { 0: -1, 1: -1, 2: -1 };
      let currentJudges = [1, 2];
      let finalResponse = currentResponse;

      const judgeReferenceGuide = `Use the following reference guide to make your judgment:

LANGUAGE & STYLE (human signals):
- Inconsistent tone — humans shift register mid-piece (formal to casual, serious to joking)
- Idiosyncratic phrasing — unusual word choices, regional slang, personal verbal tics
- Typos and grammatical quirks — minor errors, missing commas, unconventional punctuation
- Repetition of pet phrases — humans overuse certain favorite words or expressions
- Sentence rhythm variation — humans mix very short and very long sentences unpredictably

CONTENT SIGNALS (human signals):
- Specific personal anecdotes with concrete, unusual details
- References to lived experience — smells, embarrassments, family dynamics, physical sensations
- Strong, unhedged opinions — including unpopular or contradictory takes
- Niche cultural references that wouldn't appear in mainstream training data
- Internal contradictions or changing one's mind partway through

STRUCTURAL TELLS (human signals):
- Uneven organization — rambling, digression, backtracking
- Lack of perfectly balanced lists (AI loves symmetrical 3-5 item structures)
- Abrupt endings or tangents that don't resolve cleanly
- Inconsistent formatting rather than uniform bullet points and headers
...
- Lists where every item has parallel grammatical structure
- Conclusions that summarize without adding anything new
- Overly polished language with no rough edges
- Marketing-speak or buzzwords

EMOTIONAL & SOCIAL CUES (human signals):
- Genuine messy emotion — anger, pettiness, grief, joy
- Humor that risks offense or relies on shared context
- Self-deprecation or oversharing
- Inside jokes or references assuming a specific audience

IMPORTANT: No single signal is definitive — look for CLUSTERS of indicators. Be rigorous.`;
...
"""`;
        ...
        for (let j = 0; j < 2; j++) {
          const judgeIdx = currentJudges[j];
          const judgeModel = models3[judgeIdx];
          setCurrentStep(`Round ${round}: Judge (${getModelShort(judgeModel)}) evaluating...`);
        ...
        const aiJudgeLocalIndices = verdicts
          .map((v, i) => v.verdict === 'ai' ? i : -1)
          .filter(i => i >= 0);

        let hotSeatModelIdx: number;

        if (aiJudgeLocalIndices.length === 1) {
          hotSeatModelIdx = currentJudges[aiJudgeLocalIndices[0]];
        } else {
          const j0 = currentJudges[0];
          const j1 = currentJudges[1];
          hotSeatModelIdx = lastHotSeatRound[j0] <= lastHotSeatRound[j1] ? j0 : j1;
        }

        const hotSeatModel = models3[hotSeatModelIdx];
        const hotSeatLocalIdx = currentJudges.indexOf(hotSeatModelIdx);
        lastHotSeatRound[hotSeatModelIdx] = round;
...
"${verdicts[hotSeatLocalIdx].explanation}"
...
"""`;
        ...
        newSteps.push({
          ...
          content: currentResponse,
          round,
          detail: `${getModelShort(hotSeatModel)} took the hot seat`,
        });
        setSteps([...newSteps]);
        finalResponse = currentResponse;

        const otherTwo = [0, 1, 2].filter(i => i !== hotSeatModelIdx);
        currentJudges = otherTwo;
